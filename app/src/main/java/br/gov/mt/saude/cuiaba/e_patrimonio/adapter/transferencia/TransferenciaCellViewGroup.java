 ...
 */
public class TransferenciaCellViewGroup extends FrameLayout
        implements
        TableFixHeaderAdapter.FirstHeaderBinder<String>,
        TableFixHeaderAdapter.HeaderBinder<String>,
        TableFixHeaderAdapter.FirstBodyBinder<Transferencia>,
        TableFixHeaderAdapter.BodyBinder<Transferencia>,
        TableFixHeaderAdapter.SectionBinder<Transferencia> {
    ...
    public TransferenciaCellViewGroup(Context context) {
        super(context);
        this.context = context;
        init();
    }

    public TransferenciaCellViewGroup(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context = context;
        init();
    }
    ...
    @Override
    public void bindFirstBody(Transferencia item, int row) {
        textView.setText(String.valueOf(item.idtransferencia));
    ...
    @Override
    public void bindBody(Transferencia item, int row, int column) {
        Patrimonio patrimonio = patrimonioDAO.pesquisaPorPatrimonio(item.idpatrimonio);
        if(column + 1 == 1) {
            textView.setVisibility(View.GONE);
            textView2.setVisibility(View.VISIBLE);
            textView2.setText(item.idpatrimonio);
    ...
    @Override
    public void bindSection(Transferencia item, int row, int column) {
        textView.setText(column == 0 ? String.valueOf(item.idtransferencia): "");
        textView.setTypeface(null, Typeface.BOLD);
